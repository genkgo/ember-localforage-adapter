...
module("CRUD", function(hooks) {
  ...
  test("load with missing belongsTo relationships when finding a single record", function(assert) {
    ...
    run(function() {
      var p1 = store.findRecord('comment', 'c2').then(async function(comment) {
        assert.equal(null, await comment.get('post'));
      });
      var p2 = store.findRecord('comment', 'c4').then(async function(comment) {
        assert.equal(null, await comment.get('author'));
      });
  ...
  test("load with missing belongsTo relationships when finding multiple records", function(assert) {
    ...
    run(function() {
      store.findAll('comment').then(async function(comments) {
        assert.equal(null, await comments.objectAt(1).get('post'));
        assert.equal(null, await comments.objectAt(3).get('author'));
        done();
      });
    });
  });
